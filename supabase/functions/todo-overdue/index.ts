...
import { overdueEmail, sendResendEmail } from "../_shared/todo-email-templates.ts";
...
Deno.serve(async () => {
  try {
    ...
    if (error) {
      console.error("[todo-overdue] query failed:", error);
    ...
    for (const todo of todos ?? []) {
      ...
      try {
        ...
        if (profile?.todo_emails_enabled === false) {
          await supabase.from("todos").update({ overdue_sent_at: new Date().toISOString() }).eq("id", todo.id);
          results.skipped++;
          continue;
        }
        ...
        if (!result.success) {
          console.error(`[todo-overdue] resend failed for todo ${todo.id}:`, result.error);
          results.failed++;
          continue;
        }
        ...
        console.error(`[todo-overdue] error on todo ${todo.id}:`, e);
        results.failed++;
      }
    }

    console.log("[todo-overdue] run complete:", results);
    return new Response(JSON.stringify(results), {
      status: 200, headers: { "Content-Type": "application/json" },
    });
  } catch (e) {
    console.error("[todo-overdue] uncaught:", e);
